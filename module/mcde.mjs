...
// --- Build tooltip HTML once (async), then inject as data-tooltip
   ...
   if (qualities.length) {
     ...
     for (const s of spans) {
       ...
       const enriched = await TextEditor.enrichHTML(unescaped, { async: true });
...
html.find("input[name='system.chronicle_points.max']").on("change", async (ev) => {
  ...
  width: 420,
  classes: ["mcde-dialog", "mcde-trait-dialog"]
}).render(true);
    });
...
html.find(".mcde-weapon").on("drop", async (ev) => {
  ...
  width: 520,
  classes: ["mcde-dialog", "mcde-npcskilltest-dialog"]
});
...
class MCDENemesisSheet extends MCDENpcSheet {
  ...
  _mcdeGetActiveTabKey() {
    // Foundry Tabs stores active tab key
  ...
  _mcdeBindScroll(html) {
    const root = html?.[0] ?? this.element?.[0] ?? null;
  ...
  async _render(...args) {
    // Capture current scroll BEFORE rerender destroys the DOM
    this._mcdeCaptureScroll();
    await super._render(...args);
    // Restore scroll AFTER rerender
